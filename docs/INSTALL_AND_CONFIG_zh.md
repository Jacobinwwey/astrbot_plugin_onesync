...
| 当前版本 | 最后更新 | 适用对象 | 建议先读 |
| --- | --- | --- | --- |
| `v0.2.3` | `2026-04-13` | 初次安装用户、日常运维者 | [README.md](../README.md) |

这份文档给两类人看：第一次把 OneSync 装起来的人，以及已经在用、准备补目标或排障的人。它不讨论内部实现，重点是把插件跑通、把配置写对、出问题时能快速定位。

| 你现在要做什么 | 入口 |
| --- | --- |
| 首次安装并验证插件可用 | [3. 安装](#3-安装) |
| 先把 WebUI 打开 | [4.1 内置 WebUI](#41-内置-webui不改-astrbot-dashboard-源码) |
| 让 AI 直接帮你生成配置 | [5.3 AI 一键配置 Prompt 套件](#53-ai-一键配置-prompt-套件推荐) |
| 查看命令和配置字段 | [6. 命令使用](#6-命令使用) / [7. 配置总览](#7-配置总览) |
| 处理常见故障 | [11. 常见问题排查](#11-常见问题排查) |

| 相关文档 | 用途 |
| --- | --- |
| [README.md](../README.md) | 项目定位、快速开始、Prompt 模板入口 |
| [操作与同步手册（中文）](./OPERATIONS_AND_SYNC_zh.md) | 发布、同步、仓库维护 |
| [开发指南（中文）](./DEVELOPER_GUIDE_zh.md) | 代码结构、扩展点、验证流程 |
| [接口参考（中文）](./API_REFERENCE_zh.md) | `/api/*` 路由与调用顺序 |

## 1. 插件定位

`astrbot_plugin_onesync` 是一个 AstrBot 插件，负责按计划检查并更新软件。它的重点不是单个软件的升级命令，而是把多目标维护、版本验证、执行记录和 WebUI 操作收成同一套流程。
...
- 设置中心已切到摘要卡 + 分组配置结构，优先暴露模式、轮询、端口与口令状态。
- AstrBot 本地 skill 现已支持 `global / workspace` 范围管理，不再只停留在单一路径宿主视角。
...
- 指引弹窗内置“本地文档工作台”：可直接在 WebUI 浏览仓库文档，支持 `auto/zh/en/all` 语言筛选。
...
### 4.2 WebUI 内嵌 AI 助手与指引（用户/开发者）

OneSync 已将 AI 配置能力直接嵌入 WebUI，无需离开页面手工拼接 Prompt。
...
1. 打开 `AI 配置助手`，选择 Prompt 语言（中文/English）。
2. 如已存在 OneSync 配置，先点击 `从当前配置自动填充`：
   - 自动回填 `target_config_mode`、轮询参数、默认检查间隔。
   - 自动选择一个可用目标并填入对应策略字段。
   - 可选开启 `自动生成: 开`，在自动填充后直接生成 Prompt。
3. 选择场景：
   - `初始化并下发`
   - `增量新增目标`
   - `诊断与修复`
   - `完整套件`
4. 填写最少基础参数（`WEBUI_URL`、模式、轮询间隔）和一个目标的关键字段。
5. 点击 `生成 Prompt`，再点 `复制输出`，直接发送给 AI。
6. AI 返回后，按指引在配置中心保存或通过 API 下发。

用户模式建议：

1. 在 AI 助手里点击 `用户预设`，默认生成更偏向 `human` 模式的输入模板。
2. 优先使用 `初始化并下发` 场景快速起步。
3. 配置落地后，使用 `立即更新（当前筛选）` 小范围验证，再做全量。

开发者模式建议：

1. 在 AI 助手里点击 `开发者预设`，切换为 `developer + 完整套件`。
2. 让 AI 生成 `targets_json` 或 API 一键脚本。
3. 在配置中心切到 `developer` 粘贴高级配置（镜像、正则、超时等）。
4. 用 `/updater env`、`/updater check` 完成验收。

使用指引弹窗内容：

- `用户` 页签：从筛选、确认更新到日志验证的标准流程。
- `开发者` 页签：从 Prompt 生成、配置下发到环境验收的流程。
- `快捷键` 页签：热键说明与按钮等效入口说明。
- 文档直达按钮：README、安装配置手册、操作同步手册（自动按中英文切换链接）。
